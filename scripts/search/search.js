...
const DUMMY_TOURNAMENTS = [
  // STEP274:
  // 本番では初期ダミー大会を表示しない。
  // CMS / Firestore から読み込まれた大会だけがここに合流して表示される。
];
...
function catLabel(key) {
  const found = CATEGORIES.find(c => c.key === key) || CATEGORY_EXTRA.find(c => c.key === key);
...
/* ================================================================
   STEP205: CMS登録大会 → 試合検索 正式反映
   ─ CMS側が作る localStorage スナップショット
     PNX_CMS_PUBLIC_TOURNAMENTS_FOR_SEARCH を読み込み、
     既存の大会カード一覧 DUMMY_TOURNAMENTS に安全に合流させる。
   ─ 下書き/非公開は本体に出さない。
   ─ 既存ダミーデータ・カレンダー追加連携・Bottom Sheet は壊さない。
   ================================================================ */

const PNX_STEP205_CMS_PUBLIC_KEY  = 'PNX_CMS_PUBLIC_TOURNAMENTS_FOR_SEARCH';
const PNX_STEP205_CMS_RAW_KEY     = 'PNX_CMS_TOURNAMENTS';
const PNX_STEP205_CMS_META_KEY    = 'PNX_CMS_SEARCH_SYNC_META';
const PNX_STEP205_CMS_UPDATED_KEY = 'PNX_CMS_TOURNAMENTS_UPDATED_AT';

function pnxStep205Text(v) {
  return String(v == null ? '' : v).trim();
}

function pnxStep205ReadJson(key, fallback) {
  try {
    ...
    console.warn('[ProNexaX STEP205] localStorage読込失敗:', key, e);
    return fallback;
  }
}

function pnxStep205ToIsoDate(value) {
  const s = pnxStep205Text(value);
  if (!s || s === '—' || s === '不明') return '';
...
function pnxStep205HashId(text) {
  const src = pnxStep205Text(text) || 'cms-tournament';
  let h = 2166136261;
...
function pnxStep205SafeId(raw, title, start, venue) {
  const base = pnxStep205Text(raw && (raw.id || raw.tournamentId || raw.uid || raw.slug));
  if (base) {
    const cleaned = base.replace(/[^A-Za-z0-9_-]/g, '-').replace(/-+/g, '-').replace(/^-|-$/g, '');
    return cleaned.startsWith('cms-') ? cleaned : `cms-${cleaned || pnxStep205HashId(base)}`;
  }
  return `cms-${pnxStep205HashId([title, start, venue].join('|'))}`;
}

const PNX_STEP205_PREF_MAP = {
  '北海道': 'hokkaido',
  '青森': 'aomori', '青森県': 'aomori', '岩手': 'iwate', '岩手県': 'iwate', '宮城': 'miyagi', '宮城県': 'miyagi', '秋田': 'akita', '秋田県': 'akita', '山形': 'yamagata', '山形県': 'yamagata', '福島': 'fukushima', '福島県': 'fukushima',
  ...
  '新潟': 'niigata', '新潟県': 'niigata', '富山': 'toyama', '富山県': 'toyama', '石川': 'ishikawa', '石川県': 'ishikawa', '福井': 'fukui', '福井県': 'fukui', '山梨': 'yamanashi', '山梨県': 'yamanashi', '長野': 'nagano', '長野県': 'nagano', '岐阜': 'gifu', '岐阜県': 'gifu', '静岡': 'shizuoka', '静岡県': 'shizuoka', '愛知': 'aichi', '愛知県': 'aichi', '三重': 'mie', '三重県': 'mie',
  '滋賀': 'shiga', '滋賀県': 'shiga', '京都': 'kyoto', '京都府': 'kyoto', '大阪': 'osaka', '大阪府': 'osaka', '兵庫': 'hyogo', '兵庫県': 'hyogo', '奈良': 'nara', '奈良県': 'nara', '和歌山': 'wakayama', '和歌山県': 'wakayama',
...
function pnxStep205PrefKey(value) {
  const p = pnxStep205Text(value);
  if (!p) return '';
  if (PNX_STEP205_PREF_MAP[p]) return PNX_STEP205_PREF_MAP[p];
  const compact = p.replace(/[都道府県]/g, '');
  return PNX_STEP205_PREF_MAP[compact] || p.toLowerCase();
}

function pnxStep205AreaFromPref(prefKey, rawArea, rawRegion, categoryKey) {
  const area = pnxStep205Text(rawArea).toLowerCase();
  const region = pnxStep205Text(rawRegion).toLowerCase();
  if (area) {
    if (/関東|kanto/.test(area)) return 'kanto';
    if (/関西|近畿|kinki|kansai/.test(area)) return 'kinki';
    if (/中部|東海|chubu|tokai/.test(area)) return 'chubu';
    if (/九州|沖縄|kyushu|okinawa/.test(area)) return 'kyushu';
    if (/北海道|hokkaido/.test(area)) return 'hokkaido';
    if (/海外|overseas|abroad|asia|asian|pga/.test(area)) return 'overseas';
  }
  if (/海外|overseas|abroad|asia|asian|pga/.test(region) || ['abroad', 'asian', 'pga'].includes(categoryKey)) return 'overseas';
  if (DF_AREA_KEYS.kanto.includes(prefKey)) return 'kanto';
  if (DF_AREA_KEYS.kinki.includes(prefKey)) return 'kinki';
  if (DF_AREA_KEYS.chubu.includes(prefKey)) return 'chubu';
  if (DF_AREA_KEYS.kyushu.includes(prefKey)) return 'kyushu';
  if (DF_AREA_KEYS.hokkaido.includes(prefKey)) return 'hokkaido';
  return prefKey || 'all';
}

function pnxStep205NormalizeCategory(raw) {
  const c = pnxStep205Text(raw).toLowerCase();
  if (!c) return 'other';
  if (/qt|クオリ|qualifying/.test(c)) return 'qt';
  if (/pga\s*tour|米国|アメリカ|us\s*pga/.test(c)) return 'pga';
  if (/asian|アジアン|アジア/.test(c)) return 'asian';
  if (/海外|abroad|overseas|dp world|korn ferry|liv|韓国|台湾|中国|フィリピン/.test(c)) return 'abroad';
  if (/open|オープン|県オープン|地方オープン/.test(c)) return 'open';
  if (/学生|student|大学|高校/.test(c)) return 'student';
  if (/jlpga|lpga|女子プロ|ステップ/.test(c)) return 'lpga';
  if (/jgto|acn|男子ツアー|日本ゴルフツアー/.test(c)) return 'jgto';
  // 現状の検索UIではミニツアーは「その他」に集約する
  if (/mini|ミニ|future|atp|fjtour|fujisan|doucus|cartles|カートルズ/.test(c)) return 'other';
  return 'other';
}

function pnxStep205NormalizeGender(raw, categoryKey, title) {
  const s = [raw, title].map(pnxStep205Text).join(' ').toLowerCase();
  if (/女子|women|womens|female|ladies|レディス|jlpga|lpga/.test(s) || categoryKey === 'lpga') return 'womens';
  if (/男子|men|mens|male|jgto|pga|asian/.test(s) || ['jgto', 'pga', 'asian'].includes(categoryKey)) return 'mens';
  return '';
}

function pnxStep205NormalizeStatus(raw, deadline) {
  const s = pnxStep205Text(raw).toLowerCase();
  if (/ongoing|live|開催中/.test(s)) return 'live';
  if (/finished|finish|終了|closed|締切|締切済/.test(s)) return 'closed';
  if (/near|締切間近/.test(s)) return 'near';
  if (deadline) {
    const today = new Date(); today.setHours(0, 0, 0, 0);
    const d = new Date(deadline + 'T00:00:00');
    const days = Math.ceil((d - today) / 86400000);
    if (!Number.isNaN(days) && days >= 0 && days <= 14) return 'near';
    if (!Number.isNaN(days) && days < 0) return 'closed';
  }
  return 'open';
}
...
function pnxStep205NormalizeCmsTournament(raw) {
  ...
  const tags = Array.isArray(raw.tags)
    ? raw.tags.map(pnxStep205Text).filter(Boolean)
    : [];
  if (!tags.some(tag => /CMS|登録/.test(tag))) tags.push('CMS登録');

  return {
    id,
    originalCmsId: pnxStep205Text(raw.id || raw.tournamentId || ''),
    name: title,
    title,
    cat: categoryKey,
    rawCategory: pnxStep205Text(raw.category || raw.cat || ''),
    gender: pnxStep205NormalizeGender(raw.gender || raw.sex || raw.division, categoryKey, title),
    ...
    cancelDeadline: pnxStep205ToIsoDate(raw.cancelDeadline || raw.cancelDueDate) || '',
    prize: pnxStep205Text(raw.prize || raw.totalPrize || raw.prizeMoney) || '—',
    prizeWinner: pnxStep205Text(raw.prizeWinner || raw.winnerPrize || raw.winningPrize) || '—',
    entryFee: pnxStep205Text(raw.entryFee || raw.fee || raw.applicationFee) || '—',
    practiceRoundFee: pnxStep205Text(raw.practiceRoundFee || raw.playFee || raw.greenFee) || '—',
    entryMethod: pnxStep205Text(raw.entryMethod || raw.applyMethod) || (raw.entryUrl ? 'CMS登録情報 / 申込URLあり' : 'CMS登録情報を確認'),
    qualification: pnxStep205Text(raw.qualification || raw.eligibility || raw.requirement) || '—',
    capacity: pnxStep205Text(raw.capacity || raw.players || raw.fieldSize) || '—',
    ...
    addedToCalendar: APP_STATE.addedToCalendar.has(id),
    favorited: APP_STATE.favorites.has(id),
  };
}

function pnxStep205ReadCmsTournaments() {
  const publicSnapshot = pnxStep205ReadJson(PNX_STEP205_CMS_PUBLIC_KEY, null);
  const source = Array.isArray(publicSnapshot) && publicSnapshot.length
    ? publicSnapshot
    : pnxStep205ReadJson(PNX_STEP205_CMS_RAW_KEY, []);

  return (Array.isArray(source) ? source : [])
    .filter(pnxStep205IsPublicTournament)
    .map(pnxStep205NormalizeCmsTournament)
    .filter(t => t.name && t.start && t.course);
}

function PNXStep205MergeCmsTournaments(options = {}) {
  const reason = options.reason || 'manual';
  const before = DUMMY_TOURNAMENTS.length;

  const cmsItems = pnxStep205ReadCmsTournaments();
  const unique = [];
  const seen = new Set();
  ...
  const meta = pnxStep205ReadJson(PNX_STEP205_CMS_META_KEY, null);
  const result = {
    step: '205',
    reason,
    ...
    cmsCount: unique.length,
    cmsIds: unique.map(t => t.id),
    snapshotMeta: meta,
    publicKey: PNX_STEP205_CMS_PUBLIC_KEY,
  };
  window.__PNX_STEP205_CMS_SEARCH_SYNC__ = result;
  console.info('[ProNexaX STEP205] CMS大会を試合検索へ反映:', result);
  return result;
}
...
window.PNXStep205CmsSearchStatus = function() {
  return Object.assign({}, window.__PNX_STEP205_CMS_SEARCH_SYNC__ || {}, {
    publicSnapshotCount: (pnxStep205ReadJson(PNX_STEP205_CMS_PUBLIC_KEY, []) || []).length,
    rawCmsCount: (pnxStep205ReadJson(PNX_STEP205_CMS_RAW_KEY, []) || []).length,
    updatedAt: localStorage.getItem(PNX_STEP205_CMS_UPDATED_KEY) || null,
  });
};

window.addEventListener('storage', function(e) {
  if (![PNX_STEP205_CMS_PUBLIC_KEY, PNX_STEP205_CMS_RAW_KEY, PNX_STEP205_CMS_META_KEY, PNX_STEP205_CMS_UPDATED_KEY].includes(e.key)) return;
...
window.addEventListener('message', function(e) {
  ...
  if ([
    ...
    'PNX_CMS_FINAL_VALIDATED_PUBLISH_TO_APP',
    'PNX_CMS_FINAL_PUBLIC_PUBLISH_TO_APP',
    'PNX_CMS_TOURNAMENTS_UPDATED',
    'PNX_REFRESH_APP_FROM_CMS',
...
/* ================================================================
   STEP202: Search module bridge for v4.23
   ================================================================ */
(function(){
  if (window.__PNX_STEP202_SEARCH_MODULE_FIT__) return;
  window.__PNX_STEP202_SEARCH_MODULE_FIT__ = true;

  function findScrollRoot(){
    return (
      document.getElementById("pnx-search-root") ||
      document.querySelector(".pnx-search-root,.search-root,.search-page,.pnx-search-page,.tournament-search-page,.tournament-search,main") ||
  ...
  function resetSearchScroll(){
    ...
    window.__PNX_STEP202_SEARCH_LAST_SCROLL_RESET__ = new Date().toISOString();
    return true;
  }
  ...
  function init(){
    document.body.classList.add("pnx-search-embed-ready");
    notifyParentReady();
    window.__PNX_STEP202_SEARCH_LAST_INIT__ = new Date().toISOString();
    return true;
  }
  ...
  window.PNXStep202SearchResetScroll = resetSearchScroll;
  window.PNXStep202SearchBackToCalendar = requestCalendar;
  window.PNXStep202SearchModuleStatus = function(){
    const root = findScrollRoot();
    return {
      step:"202-search",
      bodyReady:document.body.classList.contains("pnx-search-embed-ready"),
      scrollRoot:root ? (root.id || root.className || root.tagName) : null,
      scrollTop:root ? root.scrollTop : null,
      innerBottomNavHidden:!!document.querySelector(".bottom-nav,.tab-bar,#tab-bar"),
