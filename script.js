...
/* ================================================================
   Cupertino Wheel Picker v3 — iOS 18 native-level JavaScript
   ================================================================ */
(function(){
'use strict';
...
/* ── Constants ── */
const ROW_H = 36;
const PAD   = 2;   // padding rows top/bottom

/* ── Deceleration physics matching iOS UIScrollView ── */
const DECEL_RATE   = 0.9935;  // iOS default deceleration rate
const MIN_VEL      = 0.5;     // px/ms threshold
const ELASTIC_K    = 0.22;    // rubber-band coefficient
...
const S = {
  ...
  allday:   false,
  interval: 15,
  start: { yr:0, mo:0, dy:0, hr:9,  mn:0 },
  end:   { yr:0, mo:0, dy:0, hr:10, mn:0 },
};

/* ── Date utilities ── */
const PAD2 = n => String(n).padStart(2,'0');
const DOW  = ['日','月','火','水','木','金','土'];
const MLAB = ['1月','2月','3月','4月','5月','6月','7月','8月','9月','10月','11月','12月'];
const daysInMonth = (y,m) => new Date(y,m,0).getDate();

function dsToObj(ds, tm){
  if(!ds) return null;
  const [y,m,d] = ds.split('-').map(Number);
  if(!y||!m||!d) return null;
  let hr=0,mn=0;
  if(tm && tm !== '__allday__'){ const [h,min]=tm.split(':').map(Number); hr=isNaN(h)?0:h; mn=isNaN(min)?0:min; }
  return {yr:y, mo:m, dy:d, hr, mn};
}
function objToDs(o){ return `${o.yr}-${PAD2(o.mo)}-${PAD2(o.dy)}`; }
function objToTm(o){ return `${PAD2(o.hr)}:${PAD2(o.mn)}`; }
function objLabel(o, allday){
  const dow = DOW[new Date(o.yr, o.mo-1, o.dy).getDay()];
  const base = `${o.mo}/${PAD2(o.dy)}(${dow})`;
  return allday ? base : `${base} ${PAD2(o.hr)}:${PAD2(o.mn)}`;
}

/* ================================================================
   Drum controller — iOS UIPickerView equivalent
   ================================================================ */
function Drum(innerId, items, initIdx){
  const el   = document.getElementById(innerId);
  const wrap = el.closest('.cpk-drum');
  let cur    = Math.max(0, Math.min(items.length-1, initIdx));
  let yOff   = -cur * ROW_H;
  let rafId  = 0;

  /* ── Apply transform (GPU-accelerated) ── */
  function _applyY(y, transition){
    el.style.transition = transition || 'none';
    const t = `translateY(${y}px) translateZ(0)`;
  ...
  function _min(){ return -(items.length-1)*ROW_H; }

  /* ── Rubber-band elastic overscroll ── */
  function _elastic(raw){
    const mn = _min();
    if(raw > 0)   return raw * ELASTIC_K;
    if(raw < mn)  return mn + (raw - mn) * ELASTIC_K;
    return raw;
  }

  /* ── Nearest snap index from pixel offset ── */
  function _idxFromY(y){
    return Math.max(0, Math.min(items.length-1, Math.round(-y/ROW_H)));
  }

  /* ── Update sel + near1/near2 classes for depth effect ── */
  function _updateClasses(idx){
    el.querySelectorAll('.cpk-cell').forEach((c,i)=>{
      const row = i - PAD;
      const dist = Math.abs(row - idx);
      c.classList.remove('sel','near1','near2');
      if(row === idx)     c.classList.add('sel');
      else if(dist === 1) c.classList.add('near1');
      else if(dist === 2) c.classList.add('near2');
    });
  }
  ...
  function _snapTo(idx, animate){
    ...
    const target = -cur * ROW_H;
    _applyY(target, animate ? 'transform .22s cubic-bezier(.25,.46,.45,.94)' : 'none');
    _updateClasses(cur);
  }
  ...
  function build(){
    ...
    _applyY(-cur*ROW_H, 'none');
    _updateClasses(cur);
  }

  /* ── Touch/Mouse input ── */
  let t0=0, y0=0, startOff=0;
  let lastY=0, lastT=0, vel=0;   // vel = px/ms
  let isDown=false;

  function _onStart(cy){
    cancelAnimationFrame(rafId);
    el.style.transition='none';
    el.style.webkitTransition='none';
    t0=Date.now(); y0=cy; startOff=yOff;
    lastY=cy; lastT=t0; vel=0;
    isDown=true;
  }
  ...
  function _onMove(cy){
    if(!isDown) return;
    const dy = cy - y0;
    const raw = startOff + dy;
    const next = _elastic(raw);
    _applyY(next, 'none');
    const now=Date.now();
  ...
  function _onEnd(){
    ...
    /* iOS UIScrollView deceleration physics */
    let v   = vel * 1000;  // convert to px/s
    let off = yOff;
    const mn = _min();
    ...
    function step(ts){
      ...
      const move = v * 0.016;  // ~60fps frame
      off += move;

      /* hard boundary */
      if(off > 0)  { off=0; v=0; }
      if(off < mn) { off=mn; v=0; }
      ...
      if(Math.abs(v) > MIN_VEL){
        ...
        /* snap */
        _snapTo(_idxFromY(off), true);
        if(typeof onChangeCallback==='function') onChangeCallback();
      }
    }

    /* if nearly stopped, snap immediately */
    if(Math.abs(v)*0.016 < 1){
      _snapTo(_idxFromY(off), true);
      if(typeof onChangeCallback==='function') onChangeCallback();
    } else {
      rafId = requestAnimationFrame(step);
  ...
  /* Touch events — passive:false to prevent scroll hijack */
  ...
  wrap.addEventListener('touchend', e=>{
    e.preventDefault(); _onEnd();
  },{passive:false});
  wrap.addEventListener('touchcancel', e=>{
    e.preventDefault(); isDown=false; _snapTo(_idxFromY(yOff),true);
  },{passive:false});

  /* Mouse (desktop) */
  let md=false;
  wrap.addEventListener('mousedown', e=>{ md=true; _onStart(e.clientY); e.preventDefault(); });
  window.addEventListener('mousemove', e=>{ if(md) _onMove(e.clientY); });
  window.addEventListener('mouseup',   e=>{ if(md){ md=false; _onEnd(); } });

  /* Scroll wheel (desktop) */
  wrap.addEventListener('wheel', e=>{
    e.preventDefault();
    cancelAnimationFrame(rafId);
    const delta = Math.sign(e.deltaY) * ROW_H;
    _snapTo(Math.max(0,Math.min(items.length-1, cur + Math.sign(delta))), true);
    if(typeof onChangeCallback==='function') onChangeCallback();
  },{passive:false});
  ...
  return {
    getIdx(){ return cur; },
    setIdx(idx, anim=false){ _snapTo(idx, anim); },
    onchange(fn){ onChangeCallback=fn; },
    rebuild(newItems, newIdx){
      items = newItems;
      cur   = Math.max(0, Math.min(items.length-1, newIdx));
      build();
    },
  };
}
...
/* ── Item list generators ── */
const yearOffset = ()=> new Date().getFullYear()-3;
function yearItems(){ const a=[]; const base=new Date().getFullYear(); for(let y=base-3;y<=base+5;y++) a.push(y+'年'); return a; }
function moItems()  { return MLAB; }
function hrItems()  { const a=[]; for(let h=0;h<24;h++) a.push(PAD2(h)+'時'); return a; }
function mnItems(iv){ const a=[]; for(let m=0;m<60;m+=iv) a.push(PAD2(m)+'分'); return a; }

/* ── 統合日付アイテム生成（月・日・曜日 1行） ── */
function dateItems(yr) {
  const today = new Date();
  const todayYr = today.getFullYear();
  const todayMo = today.getMonth() + 1;
  const todayDy = today.getDate();
  const a = [];
  for (let mo = 1; mo <= 12; mo++) {
    const days = daysInMonth(yr, mo);
    for (let dy = 1; dy <= days; dy++) {
      const isToday = (yr === todayYr && mo === todayMo && dy === todayDy);
      const dow = DOW[new Date(yr, mo - 1, dy).getDay()];
      a.push(isToday ? '今日' : `${mo}月${dy}日 ${dow}`);
    }
  }
  return a;
}
...
/* ── 日付→インデックス変換 ── */
function dateToIdx(yr, mo, dy) {
  let idx = 0;
  for (let m = 1; m < mo; m++) idx += daysInMonth(yr, m);
  return idx + (dy - 1);
}

/* ── インデックス→{mo,dy}変換 ── */
function idxToDate(yr, idx) {
  let mo = 1;
  while (mo <= 12) {
    const days = daysInMonth(yr, mo);
    if (idx < days) return { mo, dy: idx + 1 };
    idx -= days;
    mo++;
  }
  return { mo: 12, dy: 31 };
...
/* ── Full drum rebuild ── */
function buildDrums(obj){
  const fixedYr = (typeof year !== 'undefined') ? year : new Date().getFullYear();
  const mo = obj.mo || (new Date().getMonth()+1);
  const dy = obj.dy || new Date().getDate();
  const hr = obj.hr || 0;
  const mn = obj.mn || 0;
  const iv = S.interval;

  const items = dateItems(fixedYr);
  const initIdx = dateToIdx(fixedYr, mo, dy);
  drums.date = Drum('cpk-inner-date', items, initIdx);

  if(!S.allday){
    drums.hr = Drum('cpk-inner-hr', hrItems(),   hr);
    drums.mn = Drum('cpk-inner-mn', mnItems(iv), Math.round(mn/iv));
  }
}

/* ── Read current drum values ── */
function readDrums(){
  const yr = (typeof year !== 'undefined') ? year : new Date().getFullYear();
  const idx = drums.date?.getIdx() ?? 0;
  const { mo, dy } = idxToDate(yr, idx);
  const hr = S.allday ? 0 : (drums.hr?.getIdx()??0);
  const mn = S.allday ? 0 : (drums.mn?.getIdx()??0) * S.interval;
  return {yr, mo, dy, hr, mn};
}

/* ── Tab label update ── */
function refreshTabLabels(){
  document.getElementById('cpk-tab-s-val').textContent = objLabel(S.start, S.allday);
  document.getElementById('cpk-tab-e-val').textContent = objLabel(S.end,   S.allday);
}

/* ── All-day UI toggle ── */
function applyAlldayUI(){
  const timeEls = ['cpk-drum-hr','cpk-drum-mn','cpk-interval-row'];
  timeEls.forEach(id=>{
    const el=document.getElementById(id);
    if(!el) return;
    if(id==='cpk-interval-row'){
      el.style.display = S.allday ? 'none' : 'flex';
    } else {
      el.style.display = S.allday ? 'none' : '';
    }
  });
  document.getElementById('cpk-allday-btn').classList.toggle('active', S.allday);
}

/* ── Open picker ── */
window.cupertinoOpen = function(tab){
  S.tab = tab || 'start';

  const dsS = document.getElementById('input-start')?.value || '';
  const dsE = document.getElementById('input-end')?.value   || '';
  const tmS = document.getElementById('input-start-time')?.value || '';
  const tmE = document.getElementById('input-end-time')?.value   || '';

  const calYr = (typeof year !== 'undefined') ? year : new Date().getFullYear();
  const today = new Date();
  const defObj = {yr:calYr, mo:today.getMonth()+1, dy:today.getDate(), hr:9, mn:0};

  S.start = dsToObj(dsS, tmS) || defObj;
  S.end   = dsToObj(dsE, tmE) || {...defObj, hr:10};
  S.allday = (tmS === '__allday__');

  cupertinoTab(S.tab, true);
  applyAlldayUI();
  refreshTabLabels();

  document.getElementById('cpk-allday-btn').classList.toggle('active', S.allday);
  document.getElementById('cpk-overlay').classList.add('open');
};

/* ── Tab switch ── */
window.cupertinoTab = function(tab, silent){
  if(!silent && drums.date){
    /* 現在のタブの値を保存してから切り替え */
    S[S.tab] = readDrums();
    refreshTabLabels();
  }
  S.tab = tab;
  document.getElementById('cpk-tab-s').classList.toggle('active', tab==='start');
  document.getElementById('cpk-tab-e').classList.toggle('active', tab==='end');
  /* 切り替え先のタブの値でドラムを再構築 */
  buildDrums(tab==='start' ? S.start : S.end);
};

/* ── All-day toggle ── */
window.cupertinoToggleAllday = function(){
  if(drums.date){ S[S.tab] = readDrums(); }
  S.allday = !S.allday;
  applyAlldayUI();
  buildDrums(S[S.tab]);
  refreshTabLabels();
...
/* ── Minute interval change ── */
window.cupertinoSetInterval = function(iv){
  S.interval = iv;
  document.querySelectorAll('.cpk-iv').forEach(b=>{
    b.classList.toggle('active', Number(b.dataset.v)===iv);
  });
  if(!S.allday && drums.mn){
    const cur = readDrums();
    S[S.tab] = cur;
    const newMnItems = mnItems(iv);
    const nearestIdx = Math.round((cur.mn||0)/iv);
    drums.mn.rebuild(newMnItems, nearestIdx);
  }
};

/* ── Close ── */
window.cupertinoClose = function(){
  document.getElementById('cpk-overlay').classList.remove('open');
};

/* ── Apply / Done ── */
window.cupertinoApply = function(){
  /* 現在表示中のタブの値を必ず保存 */
  if(drums.date){ S[S.tab] = readDrums(); }

  function writeToInputs(obj, prefix){
    const ds = objToDs(obj);
    const el = document.getElementById(`input-${prefix}`);
    if(el) el.value = ds;
    const stEl = document.getElementById(`input-${prefix}-time`);
    if(stEl) stEl.value = S.allday ? '__allday__' : objToTm(obj);
    const dispEl = document.getElementById(`wp-${prefix}-date-display`);
    if(dispEl){
      dispEl.textContent = objLabel(obj, S.allday);
      dispEl.classList.remove('empty');
    }
    document.getElementById(`wp-${prefix}-trigger`)?.classList.add('is-set');
  }

  writeToInputs(S.start, 'start');
  writeToInputs(S.end,   'end');

  /* 終了日が開始日より前なら開始日に合わせる */
  const dsS = objToDs(S.start), dsE = objToDs(S.end);
  if(dsE < dsS){
    S.end = {...S.start, hr: Math.min(S.start.hr+1,23), mn: S.start.mn};
    writeToInputs(S.end, 'end');
  }
...
// ================================================================
const ADMIN_EMAILS = [
  // "your-email@gmail.com",  // ← 実際のメールアドレスに書き換えてください
];
// 管理者判定
...
// ゲスト用ローカルストレージをクリア（テストデータ除去）
try { localStorage.removeItem('pronexax.events.guest'); } catch(e) {}

// ユーザー固有のイベントデータ読み込み
function _loadUserEvents() {
  const key = 'pronexax.events.' + (_currentUser ? (_currentUser.uid || 'guest') : 'guest');
  try {
    ...
    if (raw) {
      ...
      if (Array.isArray(saved) && saved.length > 0) {
        ...
        // ★ nextId を読み込んだイベントの最大ID+1 に更新（ID重複防止）
        const maxId = Math.max(...userEvs.map(e => e.id), nextId - 1);
        nextId = maxId + 1;
      }
    }
  } catch(e) {}
...
// ログアウト
// authLogout は下部の完全版（⑥ authLogout）で定義
...
// 認証UIのイベントハンドラ
// ================================================================
// authSwitchTab は下部の IIFE ラッパーで拡張定義（ボタンラベル更新含む）
window.authSwitchTab = function(tab) {
  ...
  // ボタンラベルも同時更新（重複ラッパー不要化）
  var btn = document.getElementById('auth-submit-btn');
  if (btn) btn.textContent = tab === 'register' ? '新規登録' : 'ログイン';
};

// authSubmit は下部の _patchAuthSubmit（⑤）で定義
...
let year=new Date().getFullYear(),month=new Date().getMonth(),selectedDate=null,newlyAddedDate=null,modalPresetDate=null;
...
function buildChipsHTML(ds,occupiedLanes){
  ...
  let html=spacerHtml+shown.map(e=>{
    const s=e.name.length>5?e.name.slice(0,5)+'…':e.name;
    const colorStyle=e.color?` style="background:${e.color};color:#fff;"` : '';
    return`<span class="ev-chip chip-${e.cat}"${colorStyle} data-event-id="${e.id}">${s}</span>`;
  }).join('');
...
function buildCalHTML(y,m){
  ...
  const rows=Math.min(5,Math.ceil((fd+dim)/7));
  let cells=[];
...
function _fixCalendarHeight(){
  ...
  // ★ split-handle によるドラッグ操作中は flexGrow が有効なので介入しない
  //    flex:none が強制セットされていない（＝ドラッグ操作が正常に効いている）場合はスキップ
  if(wrap.style.flex !== 'none' && wrap.style.height === ''){
    return;
  }
...
function endDrag(e){
  ...
  drag.suppressClickUntil=Date.now()+400;
  drag.suppressSwipeUntil=Date.now()+600;
}
...
function initSwipe(){
  ...
  w.addEventListener('touchend',e=>{
    ...
    if(Date.now()<drag.suppressSwipeUntil){swipeLocked=false;swiping=false;return;}
...
function onCellClick(ds){const now=Date.now();if(ds===calLastClick.ds&&now-calLastClick.t<DC){calLastClick={t:0,ds:''};openCreateModal(ds);return;}calLastClick={t:now,ds};selectedDate=selectedDate===ds?null:ds;newlyAddedDate=null;renderAllPanels();renderCalList();}
function onTouchStartCell(e,ds,cellEl){lpCell=cellEl;cellEl.classList.add('longpress-active');showLpRing(e);lpTimer=setTimeout(()=>{cellEl.classList.remove('longpress-active');hideLpRing();openCreateModal(ds);},LP);}
...
function renderCalList(){
  ...
  document.getElementById('cal-event-list').innerHTML=events.map(e=>{
    ...
    const stripeStyle=e.color?` style="background:${e.color};"` : '';
    const badgeStyle=e.color?` style="background:${e.color}22;color:${e.color};"` : '';
    return`<div class="event-card${hl}${dim}" onclick="openModal(null,${e.id})"><div class="event-card-inner"><div class="event-stripe stripe-${e.cat}"${stripeStyle}></div><div class="event-content">
      ...
      ${memoHtml}
      <div class="badge-row"><span class="badge badge-${e.cat}"${badgeStyle}>${cat.label||e.cat}</span>${dlInline}${notifBtn?' '+notifBtn:''}</div>
    </div></div></div>`;
  }).join('');
}
...
function toggleSearchAdd(id){
  ...
  applySearchFilters();renderAllPanels();renderCalList();_saveUserEvents();
}
...
function goStep(n){
  ...
  else{footer.classList.remove('footer-hidden');btn.textContent=isEdit?'変更を保存する':'カレンダーに追加する';btn.disabled=false;setTimeout(function(){btn.disabled=false;},200);const backBtn=document.getElementById('btn-back-step3');if(isEdit){backBtn.style.display='none';}else{backBtn.style.display='';}}
...
function selectCatAuto(key,e){
  ...
  setTimeout(()=>{el.classList.remove('tapping');key==='personal'?(goStep(3),modalPresetDate=modalPresetDate||selectedDate,prefillForm(null)):(document.getElementById('tour-search').value='',renderTourList(),goStep(2));},250);
}
...
function prefillForm(tour){
  ...
  else{const def=defaultDateStr();['input-title','input-place','input-memo'].forEach(id=>document.getElementById(id).value='');document.getElementById('input-start').value=def;document.getElementById('input-end').value=def;
    // 時刻フィールドを終日マーカーでリセット（日付ズレ防止）
    const stEl=document.getElementById('input-start-time');const enEl=document.getElementById('input-end-time');
    if(stEl)stEl.value='__allday__';if(enEl)enEl.value='__allday__';
    document.getElementById('sel-preview').innerHTML='';
    // カラーリセット（グリーンをデフォルト）
    const colorInput=document.getElementById('input-event-color');
    if(colorInput)colorInput.value='#34C759';
    document.querySelectorAll('#color-picker-row .color-dot-wrap').forEach((el,i)=>{
      el.classList.toggle('selected',i===0);
    });
    // ★ 個人予定：ホイールピッカーの表示ボタンにも日付をセット
    if(def){
      const[_y,_m,_d]=def.split('-').map(Number);
      const _pad=n=>String(n).padStart(2,'0');
      const _fmt=`${_y}/${_pad(_m)}/${_pad(_d)}`;
      const sd=document.getElementById('wp-start-date-display');
      const ed=document.getElementById('wp-end-date-display');
      if(sd){sd.textContent=_fmt;sd.classList.remove('empty');}
      if(ed){ed.textContent=_fmt;ed.classList.remove('empty');}
      document.getElementById('wp-start-trigger')?.classList.add('is-set');
      document.getElementById('wp-end-trigger')?.classList.add('is-set');
      // WPオブジェクトも初期化しておく（ピッカーを開いた時に正しい日付から始まるように）
      if(typeof WP!=='undefined'){
        WP.startDate=new Date(_y,_m-1,_d,9,0,0,0);
        WP.endDate=new Date(_y,_m-1,_d,10,0,0,0);
        WP.isAllDay=true;
      }
    }
  }
...
function prefillFormFromEvent(ev){
  ...
  // カラー復元
  const savedColor=ev.color||'#34C759';
  const colorInput=document.getElementById('input-event-color');
  if(colorInput)colorInput.value=savedColor;
  document.querySelectorAll('#color-picker-row .color-dot-wrap').forEach(el=>{
    el.classList.toggle('selected',el.dataset.color===savedColor);
  });
...
function checkSubmit(){document.getElementById('modal-next-btn').disabled=false;}
function syncEndDate(){const s=document.getElementById('input-start').value,e=document.getElementById('input-end');if(s&&(!e.value||e.value<s))e.value=s;}
...
/* ── カラー選択 ── */
function _selectEventColor(el) {
  document.querySelectorAll('#color-picker-row .color-dot-wrap').forEach(d=>d.classList.remove('selected'));
...
function submitEvent(){
  ...
  const eventColor=document.getElementById('input-event-color')?.value||'#34C759';
  if(!title){showToast('予定名を入力してください');return;}
  if(!selectedCat||!start)return;
  if(editingId){
    ...
    if(idx>=0){const old=EVENTS[idx];const oldDb=DB[old.cat]||[];const oldIdx=oldDb.findIndex(t=>t.name===old.name);if(oldIdx>=0)addedIds.delete(`${old.cat}-${oldIdx}`);
      EVENTS[idx]={...EVENTS[idx],name:title,date:start,end:end,place,memo,cat:selectedCat,color:eventColor};
      const newDb=DB[selectedCat]||[];const newIdx=newDb.findIndex(t=>t.name===title);if(newIdx>=0)addedIds.add(`${selectedCat}-${newIdx}`);}
    closeModal();newlyAddedDate=start;selectedDate=start;modalPresetDate=null;renderAllPanels();renderCalList();_saveUserEvents();showToast(`「${title}」を更新しました ✎`);
  } else {
    ...
    closeModal();newlyAddedDate=start;selectedDate=start;modalPresetDate=null;renderAllPanels();renderCalList();_saveUserEvents();showToast(`「${title}」を追加しました`);
  }
}
// ════════════════════════════════════════════════════════════════
// ★★★ エントリー締切＆通知機能 ★★★
...
function showToast(msg){const t=document.getElementById('toast');if(!t)return;t.classList.remove('show');void t.offsetWidth;t.textContent=msg;t.classList.add('show');setTimeout(()=>t.classList.remove('show'),2400);}
...
function setSplitRatio(calRatio,animate){
  ...
  // ★ _fixCalendarHeight が flex:none を強制セットしている場合は解除してからflexGrowを設定
  if(cal.style.flex==='none'||cal.style.height){
    cal.style.height='';
    cal.style.minHeight='';
    cal.style.flex='';
  }
...
(function restoreDesign(){
  try{
    ...
    if(['light','dark','system'].includes(obj.theme))designConfig.theme='light'; // 常にライト固定
...
submitEvent=function(){
  // タイトル未入力チェック
  const _title=document.getElementById('input-title')?.value.trim();
  if(!_title){showToast('予定名を入力してください');return;}

  const stEl=document.getElementById('input-start-time');
  const enEl=document.getElementById('input-end-time');
  const startDate=document.getElementById('input-start').value;
  const endDate=document.getElementById('input-end').value||startDate;
  const stT=(stEl?stEl.value:'').replace('__allday__','');
  const enT=(enEl?enEl.value:'').replace('__allday__','');
...
(function(){
  ...
  window.prefillForm=function(tour){
    // ★ スマホ対応：_origPrefill呼び出し前にselectedDateをキャプチャ（最も確実なタイミング）
    const _capturedDate=(typeof selectedDate!=='undefined'&&selectedDate)?selectedDate
                       :(typeof modalPresetDate!=='undefined'&&modalPresetDate)?modalPresetDate:null;
    ...
    setTimeout(()=>{
      ...
      if(!isEdit&&window._rangeEndPreset&&!tour){
        ...
        if(inS&&inE){
          // 開始日はキャプチャ済みselectedDateから（スマホ遅延対策）
          const startDs=_capturedDate;
      ...
      if(isEdit){
        ...
        // 新規：キャプチャ済みselectedDateを最優先（スマホでの遅延対策）
        baseDate=parseDateLocalStr(_capturedDate||inS?.value);
      }
...
async function startUrlFetch() {
  if (typeof window._aiStartFetch === 'function') {
    return window._aiStartFetch();
  }
  _showUrlError('解析エンジンを読み込み中です。少し待ってから再度お試しください。');
}
...
function confirmUrlResult() {
  ...
  // closeUrlModal() は _urlParseResult = null するため、先にコピーを退避
  const _data = _urlParseResult;
  // モーダルを閉じる（null 化される前にコピー済み）
  document.getElementById('url-modal-overlay')?.classList.remove('show');
  _urlParseResult = null;
  // 解析結果をフォームに pre-fill
  _atmInitSelects();
  ...
  _atmClear();
  _atmFill(_data);
  _atmSet('atm-sourceUrl',  _data.sourceUrl);
  _atmSet('atm-sourceType', _data.sourceType);
  document.getElementById('atm-overlay')?.classList.add('show');
  document.getElementById('atm-sheet')?.classList.add('show');
}
...
const TextCleaner = (function() {
  ...
  // フッター・ナビゲーション的な行
  const FOOTER_RE = /^.{0,20}(Copyright|©|プライバシーポリシー|利用規約|サイトマップ|会社概要|採用情報).{0,20}$/gim;
  ...
  function _domExtract(html) {
    try {
      ...
      if (priority.replace(/\s/g, '').length < 500) return body;
      return priority;
    } catch {
...
class AiUrlParser {
  ...
  // 旧プロキシルート削除済み — HTML取得はサーバーサイド(/api/parseTournament)で完結
  ...
  /* ── HTML フェッチ（旧ブラウザ直接fetch廃止） ──
   * HTML取得はサーバーサイド(/api/parseTournament)で行う。
   * ブラウザからは pageText を空のまま callApi に渡し、
   * サーバーが sourceUrl を受け取ってfetchする。
   */
  static async fetchHtml(_url) {
    return '';
  }

  /* ── Browser Render Engine（iframe DOM取得）──────────────────────
   * 用途: Cloudflare 保護サイト / SPA / JS描画必須ページ
   *
   * 処理:
   *   1. hidden iframe を生成・body に追加
   *   2. DOMContentLoaded または load のどちらか早い方を待つ
   *   3. iframe.contentDocument から innerHTML を取得
   *   4. iframe を必ず削除（メモリ・バッテリー保護）
   *
   * モバイル PWA 対策:
   *   - RENDERED_TIMEOUT_MS: 8秒でタイムアウト（ハング防止）
   *   - document.hidden: バックグラウンド時はスキップ
   *   - finally: 失敗時も iframe を確実に除去
   *   - X-Frame-Options / CSP ブロック: try/catch で静かに失敗
   *
   * 戻り値: HTML文字列（取得失敗時は ''）
   */
  static RENDERED_TIMEOUT_MS = 8000;

  static async fetchRenderedHtml(url) {
    // バックグラウンド実行は skip（バッテリー節約）
    if (typeof document === 'undefined' || document.hidden) {
      console.info('[BrowserRender] skip — document.hidden or undefined:', url);
      return '';
    }

    console.info('[BrowserRender] ▶ start:', url);

    return new Promise((resolve) => {
      let iframe = null;
      let settled = false;

      // ── cleanup: iframe を DOM から確実に除去（zombie 防止）──
      const cleanup = () => {
        if (iframe) {
          try { iframe.remove(); } catch {}
          iframe = null;
          console.info('[BrowserRender] iframe removed (cleanup)');
        }
      };

      // ── done: 二重 resolve 防止 + cleanup 保証 ──
      const done = (html) => {
        if (settled) return;
        settled = true;
        cleanup();
      ...
      // ── タイムアウト（RENDERED_TIMEOUT_MS = 8s）──
      const timer = setTimeout(() => {
        console.warn('[BrowserRender] ⏱ timeout (8s) — iframe 強制除去:', url);
        done('');
      }, this.RENDERED_TIMEOUT_MS);
      ...
      try {
        ...
        console.info('[BrowserRender] iframe created, src=', url);
        ...
        const extract = () => {
          ...
          try {
            ...
            if (!doc || !doc.body) {
              console.warn('[BrowserRender] doc/body が null — 取得不可:', url);
              done(''); return;
            }
            const html = doc.body.innerHTML || doc.documentElement?.outerHTML || '';
            console.info(
              `[BrowserRender] ✅ DOM extracted: ${html.length} chars` +
              ` | body.children: ${doc.body.children.length}` +
              ` | title: "${doc.title}"` +
              ` | url: ${url}`
            );
            // iframe が残っていないことを確認（done 内の cleanup で除去済みのはず）
            if (document.body.contains(iframe)) {
              console.warn('[BrowserRender] ⚠ iframe まだ DOM に残存 — 強制除去します');
            }
            done(html);
          } catch (e) {
            // X-Frame-Options / CSP でクロスオリジンアクセス不可
            console.warn(
              '[BrowserRender] ❌ cross-origin block — DOM アクセス不可',
              `| error: ${e.message}`,
              `| url: ${url}`
            );
            done('');
        ...
        iframe.addEventListener('DOMContentLoaded', () => {
          console.info('[BrowserRender] DOMContentLoaded fired:', url);
          extract();
        }, { once: true });

        iframe.addEventListener('load', () => {
          console.info('[BrowserRender] load fired:', url);
          extract();
        }, { once: true });

        iframe.src = url;
        document.body.appendChild(iframe);
        console.info('[BrowserRender] iframe appended to body');

      } catch (e) {
        clearTimeout(timer);
        console.warn('[BrowserRender] ❌ iframe 生成失敗:', e.message, url);
        cleanup();
        resolve('');
      }
    });
  ...
  /* ── Server-side Render Fetch Engine ────────────────────────────
   * 用途: cross-origin / X-Frame-Options / SPA で
   *       fetchHtml・fetchRenderedHtml が両方失敗したサイト向け。
   *
   * 処理:
   *   POST /api/renderFetch { url }
   *   → サーバー側 headless browser (Puppeteer/Playwright/Browserless)
   *   → networkidle 待ち → rendered HTML → JSON 返却
   *
   * クライアント側の責務はリクエスト1本のみ。
   * headless browser の起動・終了・cleanup はすべてサーバー側。
   * スマホ battery / memory / cold start への影響ゼロ。
   *
   * タイムアウト: RENDER_FETCH_TIMEOUT_MS = 20s
   *   Vercel Function の maxDuration: 30s に対応。
   *   cold start 込みで 15〜20s を想定。
   */
  static RENDER_FETCH_ROUTE      = '/api/renderFetch';
  static RENDER_FETCH_TIMEOUT_MS = 20000;

  static async serverRenderFetch(url) {
    console.info('[ServerRender] ▶ start:', url);
    let res;
    try {
      res = await fetch(this.RENDER_FETCH_ROUTE, {
        method:  'POST',
        headers: { 'Content-Type': 'application/json' },
        body:    JSON.stringify({ url }),
        signal:  AbortSignal.timeout(this.RENDER_FETCH_TIMEOUT_MS),
      });
    } catch (e) {
      console.warn('[ServerRender] ❌ fetch 失敗:', e.message, url);
      return '';
    }
    if (!res.ok) {
      console.warn(`[ServerRender] ❌ HTTP ${res.status} — ${url}`);
      return '';
    }
    let body;
    try { body = await res.json(); } catch (e) {
      console.warn('[ServerRender] ❌ JSON parse 失敗:', e.message);
      return '';
    }
    if (!body.ok || !body.html) {
      console.warn('[ServerRender] ❌ サーバーエラー:', body.error || 'no html', url);
      return '';
    }
    console.info(`[ServerRender] ✅ 取得成功: ${body.html.length} chars | url: ${url}`);
    return body.html;
  }
  ...
  static async parse(url, onStep = () => {}) {
    ...
    // ── STEP 1: HTML フェッチ ──
    s('fetch', 'active');
    let rawHtml = await this.fetchHtml(url);
    s('fetch', rawHtml ? 'done' : 'err');

    // ── STEP 1-b: Browser Render Engine（fetchHtml が空のとき試みる）──
    // サーバーサイド fetch が失敗 / Cloudflare 保護 / SPA 等で rawHtml が空の場合、
    // hidden iframe でブラウザにレンダリングさせて DOM を取得する。
    // 取得できた場合のみ rawHtml を上書き。失敗時は静かに次へ（URL-only AI fallback）。
    if (!rawHtml) {
      console.info('[AI] fetchHtml → empty — Browser Render Engine を試みます:', url);
      try {
        const rendered = await this.fetchRenderedHtml(url);
        if (rendered && rendered.length > 200) {
          rawHtml = rendered;
          console.info(`[AI] ✅ rendered fallback 成功: ${rendered.length} chars → STEP 2 へ`);
          s('fetch', 'done');
        } else {
          console.info(
            `[AI] rendered fallback: 取得不可 (length=${rendered ? rendered.length : 0})` +
            ' → URL-only AI fallback へ'
          );
        }
      } catch (e) {
        console.warn('[AI] Browser Render Engine 例外:', e.message, '→ serverRenderFetch へ');
      }
    } else {
      console.info(`[AI] fetchHtml 成功: ${rawHtml.length} chars — rendered fallback スキップ`);
    }

    // ── STEP 1-c: Server-side Render Fetch（A・B が両方失敗のとき試みる）──
    // Puppeteer/Playwright/Browserless による headless browser fetch。
    // クライアントは POST /api/renderFetch を投げるだけ。
    // バッテリー・メモリ負荷ゼロ（サーバー側で処理）。
    if (!rawHtml) {
      console.info('[AI] Browser Render Engine も空 — Server Render Fetch を試みます:', url);
      try {
        const serverHtml = await this.serverRenderFetch(url);
        if (serverHtml && serverHtml.length > 200) {
          rawHtml = serverHtml;
          console.info(`[AI] ✅ serverRenderFetch 成功: ${serverHtml.length} chars → STEP 2 へ`);
          s('fetch', 'done');
        } else {
          console.info(
            `[AI] serverRenderFetch: 取得不可 (length=${serverHtml ? serverHtml.length : 0})` +
            ' → URL-only AI fallback へ'
          );
        }
      } catch (e) {
        console.warn('[AI] serverRenderFetch 例外:', e.message, '→ URL-only AI fallback へ');
      }
    }
    ...
    const _hintLines = [
      ...
      _fmtSimple(candidates.candidateOrganizers,     '主催者'),
      _fmtSimple(candidates.candidateCapacities,     '定員'),
      _fmtSimple(candidates.candidateQualifications, '出場資格'),
      _fmtSimple(candidates.candidateEntryUrls,      'エントリーURL'),
      _fmtSimple(candidates.candidateIgUrls,         'InstagramURL'),
    ].filter(Boolean);
  ...
  static extractCandidates(text, domIndex = new Map()) {
    ...
    const _orgRaw = [];
    const ORG_RE = /[^
]*(?:主催|主催者|オーガナイザー|organizer)[^
]*/gi;
    let om;
    while ((om = ORG_RE.exec(text)) !== null && _orgRaw.length < 6) {
      const line = om[0].trim();
      if (line.length > 3 && line.length < 60) {
        const val = line.replace(/^.*?[:：]/,'').trim() || line;
        _orgRaw.push({ value: val.slice(0,40), sourceLine: line.slice(0,80) });
      }
    }
    const candidateOrganizers = _rank(_orgRaw, 'organizer').slice(0, 2);
    ...
    let capm;
    while ((capm = CAP_RE.exec(text)) !== null && _capRaw.length < 6) {
      const line = capm[0].trim();
      const numM = line.match(/\d+\s*(?:名|人|組)/);
      if (numM) _capRaw.push({ value: numM[0], sourceLine: line.slice(0,80) });
    }
    const candidateCapacities = _rank(_capRaw, 'capacity').slice(0, 2);

    /* ── ⑧ 出場資格 ────────────────────────────────────────── */
    const _qualRaw = [];
    const QUAL_RE = /[^
]*(?:出場資格|参加資格|応募資格|対象者|資格)[^
]*/gi;
    let qm;
    while ((qm = QUAL_RE.exec(text)) !== null && _qualRaw.length < 6) {
      const line = qm[0].trim();
      if (line.length > 5 && line.length < 100) {
        const val = line.replace(/^.*?[:：]/,'').trim() || line;
        _qualRaw.push({ value: val.slice(0,60), sourceLine: line.slice(0,100) });
      }
    }
    const candidateQualifications = _rank(_qualRaw, 'qualification').slice(0, 2);

    /* ── ⑨ エントリーURL ────────────────────────────────────── */
    const _entryUrlRaw = [];
    const ENTRY_URL_RE = /https?:\/\/[^\s"'<>]{10,120}/g;
    const ENTRY_LABEL_RE = /(?:エントリー|申込|申し込み|entry|form|register)/i;
    let eum;
    while ((eum = ENTRY_URL_RE.exec(text)) !== null && _entryUrlRaw.length < 6) {
      const url = eum[0];
      if (ENTRY_LABEL_RE.test(url) || ENTRY_LABEL_RE.test(text.slice(Math.max(0,eum.index-30), eum.index))) {
        _entryUrlRaw.push({ value: url, sourceLine: url });
      }
    }
    const candidateEntryUrls = _entryUrlRaw.slice(0, 2);

    /* ── ⑩ Instagram URL ────────────────────────────────────── */
    const _igRaw = [];
    const IG_RE = /https?:\/\/(?:www\.)?instagram\.com\/[^\s"'<>]{2,60}/g;
    let igm;
    while ((igm = IG_RE.exec(text)) !== null && _igRaw.length < 3) {
      _igRaw.push({ value: igm[0], sourceLine: igm[0] });
    }
    const candidateIgUrls = _igRaw.slice(0, 2);
    ...
    console.log('[AIP:candidates] bestCandidate(text+dom):', {
      ...
      organizer:    candidateOrganizers[0]?.value   || '-',
      capacity:     candidateCapacities[0]?.value   || '-',
      qualification:candidateQualifications[0]?.value || '-',
      entryUrl:     candidateEntryUrls[0]?.value    || '-',
      instagram:    candidateIgUrls[0]?.value       || '-',
    });

    return {
      candidateDates, candidateDeadlines, candidatePrizes,
      candidateFees, candidateCourses,
      candidateOrganizers, candidateCapacities, candidateQualifications,
      candidateEntryUrls, candidateIgUrls,
    };
  }
...
// ================================================================
// ★ URL解析 複数大会一括登録（AiUrlParser 定義後に配置）
// ================================================================
let _aiResults = [];

window._aiStartFetch = async function() {
  console.log('[AI] _aiStartFetch called');
  const url = document.getElementById('url-modal-input')?.value?.trim();
  if (!url || !url.startsWith('http')) {
    _aiShowError('有効なURLを入力してください（https://... ）');
    return;
  }
  ...
  try {
    const aiRaw = await AiUrlParser.parse(url, (step) => {
      const map = { fetch:'ページ取得中...', parse:'テキスト解析中...', ai:'AI解析中...', cat:'カテゴリ判定中...' };
      if (map[step]) _setUrlProgress(60, map[step]);
    });

    const list = Array.isArray(aiRaw) ? aiRaw : (aiRaw ? [aiRaw] : []);
    if (!list.length) throw new Error('大会情報を抽出できませんでした');

    _setUrlProgress(100, `${list.length}件の大会を抽出しました`);
    _aiResults = list.map(r => ({ ...r, _srcUrl: url }));
    _aiShowList(_aiResults);

  } catch(e) {
    console.warn('[AI URL]', e);
    _aiShowError(e.message || '解析に失敗しました');
  } finally {
    btn.disabled = false;
  }
};
...
function _aiHideAll() {
  ['url-progress','url-result-box','url-error-box'].forEach(id => {
    document.getElementById(id)?.classList.remove('show');
  });
  document.getElementById('url-confirm-btn').style.display = 'none';
  document.getElementById('url-bulk-btn').style.display   = 'none';
  _setUrlProgress(0, '');
}
...
function _aiShowList(list) {
  ...
  rows.innerHTML =
    `<div class="ai-tour-header">
       <span class="ai-tour-count">${list.length}件の大会が見つかりました</span>
       <span class="ai-tour-selall" onclick="_aiToggleAll()">すべて選択</span>
     </div>
     <div class="ai-tour-list" id="ai-tour-list">
       ${list.map((t, i) => {
         const nm  = escapeHtml(t.name || t.title || '(大会名不明)');
         const s   = t.start || t.startDate || '';
         const e   = t.end   || t.endDate   || '';
         const pl  = escapeHtml(t.place || t.course || '');
         const cat = escapeHtml(t.category || '');
         const pr  = escapeHtml(t.prize || '');
         const fe  = escapeHtml(t.entryFee || '');
         const chips = [
           s  ? `<span class="ai-tour-chip">📅 ${s}${e && e!==s?' 〜 '+e:''}</span>` : '',
           pl ? `<span class="ai-tour-chip">📍 ${pl}</span>` : '',
           cat? `<span class="ai-tour-chip">${cat}</span>` : '',
           pr ? `<span class="ai-tour-chip">💰 ${pr}</span>` : '',
           fe ? `<span class="ai-tour-chip">💳 ${fe}</span>` : '',
         ].filter(Boolean).join('');
         return `<div class="ai-tour-item on" data-i="${i}" onclick="_aiToggle(this)">
           <div class="ai-tour-cb">✓</div>
           <div>
             <div class="ai-tour-name">${nm}</div>
             <div class="ai-tour-meta">${chips || '<span class="ai-tour-chip">情報少</span>'}</div>
           </div>
         </div>`;
       }).join('')}
     </div>`;
  ...
  document.getElementById('url-error-box').classList.remove('show');
  document.getElementById('url-confirm-btn').style.display = 'none';
  const bulkBtn = document.getElementById('url-bulk-btn');
  bulkBtn.style.display = '';
  _aiUpdateBtn();
}

window._aiToggle = function(el) { el.classList.toggle('on'); _aiUpdateBtn(); };
window._aiToggleAll = function() {
  const items = [...document.querySelectorAll('#ai-tour-list .ai-tour-item')];
  const allOn = items.every(el => el.classList.contains('on'));
  items.forEach(el => allOn ? el.classList.remove('on') : el.classList.add('on'));
  _aiUpdateBtn();
};
function _aiUpdateBtn() {
  const btn = document.getElementById('url-bulk-btn');
  if (!btn) return;
  const n = document.querySelectorAll('#ai-tour-list .ai-tour-item.on').length;
  btn.textContent = n ? `選択した ${n} 件を追加` : '大会を選択してください';
  btn.disabled    = !n;
}

window._aiBulkRegister = function() {
  const checked = [...document.querySelectorAll('#ai-tour-list .ai-tour-item.on')];
  const sel = checked.map(el => _aiResults[+el.dataset.i]).filter(Boolean);
  if (!sel.length) { showToast('大会を選択してください'); return; }
  ...
  sel.forEach(t => {
    ...
    const end   = t.end || t.endDate || start;
    const cat   = t.category || 'personal';
    const memo  = [
      t.qualification ? `出場資格: ${t.qualification}` : '',
      t.capacity      ? `定員: ${t.capacity}`          : '',
      t.organizer     ? `主催: ${t.organizer}`          : '',
      t.entryFee      ? `参加費: ${t.entryFee}`         : '',
      t.prize         ? `賞金: ${t.prize}`              : '',
      t.entryUrl      ? `エントリー: ${t.entryUrl}`     : '',
      t.instagram     ? `Instagram: ${t.instagram}`     : '',
      t.memo || t.notes || '',
    ].filter(Boolean).join('\n');
  ...
  if (added) {
    ...
    renderCalList();
    _saveUserEvents();
    showToast(`✅ ${added}件の大会を追加しました`);
  } else {
    showToast('追加できませんでした（大会名・開始日が必要）');
  }
};
...
function showResult(data) {
  ...
  const fields = [
    ...
    ['締切',         data.entryDeadline],
    ['キャンセル締切',data.cancelDeadline],
    ['ゴルフ場',     data.course || data.courseName],
    ['都道府県',     pref],
    ['主催者',       data.organizer],
    ['賞金総額',     data.prize],
    ['優勝賞金',     data.prizeWinner],
    ['参加費',       data.entryFee],
    ['練習ラウンド費',data.practiceRoundFee],
    ['定員',         data.capacity],
    ['出場資格',     data.qualification],
    ['エントリー方法',data.entryMethod],
    ['公式URL',      data.links?.official || data.officialUrl],
    ['エントリーURL', data.links?.entry || data.entryUrl],
...
window.pdfFromUrl = async function() {
  ...
  let buf;
  try {
    // 旧 allorigins プロキシ削除済み — 直接fetchのみ（CORSヘッダー必須）
    const res = await fetch(url, { signal: AbortSignal.timeout ? AbortSignal.timeout(15000) : undefined });
...
function _onSelect(key) {
  ...
  setTimeout(function(){
    ...
    if (key === 'personal') {
      /* 個人予定 → step3（詳細入力）へ直接 */
      /* ★ バグ修正: prefillForm(null) が呼ばれる前に input-start / input-end へ
         ダブルクリック日付(_presetDs)を直接セットする。
         defaultDateStr() が古い値 or null を返すケースでも確実に日付が入る。 */
      const _inS = document.getElementById('input-start');
      const _inE = document.getElementById('input-end');
      if (_inS && _presetDs) _inS.value = _presetDs;
      if (_inE && _presetDs && !_inE.value) _inE.value = _presetDs;
  ...
  function patch(){
    ...
    window.openModal = function(ds, editId){
      if (editId) {
        ...
        /* 新規追加 → state だけセットして catsel を開く（_origは呼ばない） */
        const ds2 = (ds && typeof ds === 'string') ? ds : (typeof todayStr === 'function' ? todayStr() : new Date().toISOString().slice(0,10));
        if (typeof window.modalPresetDate !== 'undefined') window.modalPresetDate = ds2;
        if (typeof window.selectedDate    !== 'undefined') window.selectedDate    = ds2;
        if (typeof window.editingId       !== 'undefined') window.editingId       = null;
        selectedCat  = null;
        selectedTour = null;
        /* modal-overlay は catsel 表示中は非表示のまま */
        const mainOv = document.getElementById('modal-overlay');
        if (mainOv) mainOv.classList.remove('show');
        catselOpen();
      }
...
/* ================================================================
   ProNexaX — URL Discovery Engine v1.0
   大会一覧ページから詳細URLを自動検出し watchTargets へ追加する。

   接続: discoverTournamentUrls()
           └─ TournamentWatcher.addTarget()  ← Watcher側は無変更
   dedupe:
     Layer1  normalizeUrl() 後に Set で即時重複排除
     Layer2  TournamentWatcher.addTarget() 内の Firestore where('url','==')
   ================================================================ */
class UrlDiscoveryEngine {

  // ── スコアリングルール ──────────────────────────────────────────
  static SCORE_RULES = [
    { re: /\/tournament\//i,    score: +10 },
    { re: /\/taikai\//i,        score: +10 },
    { re: /\/event\//i,         score:  +8 },
    { re: /\/competition\//i,   score:  +8 },
    { re: /\/entry\//i,         score:  +6 },
    { re: /\/schedule\//i,      score:  +5 },
    { re: /\/detail\//i,        score:  +5 },
    { re: /\/info\//i,          score:  +3 },
    { re: /\/result\//i,        score:  +3 },
    { re: /\/202[5-9]\//i,      score:  +4 },
    { re: /\/news\//i,          score:  -5 },
    { re: /\/sponsor\//i,       score: -10 },
    { re: /\/about\//i,         score:  -8 },
    { re: /\/contact\//i,       score:  -8 },
    { re: /\/privacy\//i,       score: -10 },
    { re: /\/sitemap/i,         score: -10 },
    { re: /\/login\b/i,         score: -10 },
    { re: /\/search\b/i,        score:  -5 },
  ];

  static SCORE_THRESHOLD = 0;
  static MAX_RESULTS     = 50;

  // ── URL 正規化 ─────────────────────────────────────────────────
  // utm削除 / hash削除 / relative→absolute / trailing slash統一
  static normalizeUrl(href, baseUrl) {
    if (!href) return null;
    try {
      ...
      // utm_* / fbclid / ref 除去
      const drop = [...abs.searchParams.keys()].filter(k =>
        k.startsWith('utm_') || k === 'fbclid' || k === 'ref');
      drop.forEach(k => abs.searchParams.delete(k));
      abs.search = abs.searchParams.toString() ? '?' + abs.searchParams.toString() : '';

      // hash 除去
      abs.hash = '';

      // trailing slash 統一（ルート以外は除去）
  ...
  static scoreUrl(url) {
    let score = 0;
    for (const r of this.SCORE_RULES) {
      if (r.re.test(url)) score += r.score;
    }
    return score;
  }

  // ── 除外判定（バイナリ・画像・PDF 等）──────────────────────────
  static _isExcluded(url) {
    return /\.(pdf|jpg|jpeg|png|gif|svg|webp|mp4|zip|docx|xlsx)(\?|$)/i.test(url)
        || /\/(tag|category|author|page\/\d)(\/?$|\/)/i.test(url);
  }

  // ── HTML 取得（AiUrlParser.fetchHtml を再利用）─────────────────
  // 旧 allorigins フォールバック削除済み — AiUrlParser が必須
  static async fetchHtml(url) {
    if (typeof AiUrlParser !== 'undefined' && typeof AiUrlParser.fetchHtml === 'function') {
      return AiUrlParser.fetchHtml(url);
    }
    throw new Error('[Discovery] AiUrlParser が未定義のため HTML 取得不可');
  }

  // ── a[href] 全取得 → 正規化 → スコアリング → dedup ───────────
  static extractLinks(html, baseUrl) {
    if (!html) return [];
    let doc;
    try { doc = new DOMParser().parseFromString(html, 'text/html'); }
    catch(e) { console.warn('[Discovery] DOMParser失敗:', e.message); return []; }
    ...
    doc.querySelectorAll('a[href]').forEach(a => {
      ...
      if (!url || this._isExcluded(url) || seen.has(url)) return;
      seen.add(url);

      const score = this.scoreUrl(url);
      if (score < this.SCORE_THRESHOLD) return;
    ...
    return links.slice(0, this.MAX_RESULTS);
  }

  // ================================================================
  //  discoverTournamentUrls(listingUrl, options)
  //
  //  処理:
  //    1. fetchHtml(listingUrl)
  //    2. DOMParser + a[href] 全取得
  //    3. normalizeUrl()
  //    4. scoreUrl() / 重複除外
  //    5. TournamentWatcher.addTarget() で watchTargets へ自動追加
  //       ※ discoveredAt / discoverySource フィールドを追加
  //
  //  options:
  //    dryRun        (bool)   true = 書き込みなしで結果だけ返す
  //    scoreMin      (number) このスコア以上のみ登録（default: 0）
  //    maxAdd        (number) 今回の上限数（default: 20）
  //    intervalHours (number) Watcher 巡回間隔（default: 24）
  //    onProgress    (fn)     ({ done, total, url, result }) コールバック
  //
  //  戻り値:
  //    { ok, listingUrl, discovered, added, updated, skipped, urls }
  // ================================================================
  static async discoverTournamentUrls(listingUrl, options = {}) {
    const {
      dryRun        = false,
      scoreMin      = 0,
      maxAdd        = 20,
      intervalHours = (typeof TournamentWatcher !== 'undefined' ? TournamentWatcher.DEFAULT_HOURS : 24),
      onProgress    = null,
    } = options;

    if (!listingUrl?.startsWith('http')) {
      return { ok: false, error: 'Invalid listingUrl', listingUrl, discovered: 0, added: 0, updated: 0, skipped: 0, urls: [] };
    }

    console.log(`[Discovery] 開始: ${listingUrl} (dryRun=${dryRun}, scoreMin=${scoreMin})`);

    // 1. HTML 取得
    let html;
    try {
      html = await this.fetchHtml(listingUrl);
    } catch(e) {
      console.error('[Discovery] fetchHtml失敗:', e.message);
      return { ok: false, error: e.message, listingUrl, discovered: 0, added: 0, updated: 0, skipped: 0, urls: [] };
    }

    // 2-4. リンク抽出・スコアリング・dedupe(layer1)
    const all      = this.extractLinks(html, listingUrl);
    const filtered = all.filter(l => l.score >= scoreMin).slice(0, maxAdd);
    console.log(`[Discovery] 発見: ${all.length}件 → score≥${scoreMin}: ${filtered.length}件`);

    // dryRun: 書き込みなしで終了
    if (dryRun) {
      console.log('[Discovery] dryRun — 書き込みスキップ');
      return { ok: true, listingUrl, discovered: all.length, added: 0, updated: 0, skipped: filtered.length,
               urls: filtered.map(l => ({ ...l, addResult: { dryRun: true } })) };
    }

    // TournamentWatcher が未定義の場合
    if (typeof TournamentWatcher === 'undefined') {
      console.warn('[Discovery] TournamentWatcher 未定義 → Firestore 登録スキップ');
      return { ok: false, error: 'TournamentWatcher undefined', listingUrl,
               discovered: all.length, added: 0, updated: 0, skipped: filtered.length,
               urls: filtered.map(l => ({ ...l, addResult: { skipped: true } })) };
    }

    // 5. watchTargets へ追加（dedupe layer2 は addTarget 内の where クエリが担当）
    ...
    for (let i = 0; i < filtered.length; i++) {
      ...
      let addResult;
      try {
        addResult = await TournamentWatcher.addTarget(link.url, {
          source:          'discovery',
          discoverySource: listingUrl,
          discoveredAt:    new Date().toISOString(),
          intervalHours,
          enabled: true,
        });
        if (addResult.added)   added++;
        if (addResult.updated) updated++;
        if (!addResult.ok)     failed++;
      } catch(e) {
        console.error('[Discovery] addTarget失敗:', link.url, e.message);
        addResult = { ok: false, error: e.message };
        failed++;
      }

      results.push({ ...link, addResult });
      if (typeof onProgress === 'function') {
        try { onProgress({ done: i + 1, total: filtered.length, url: link.url, result: addResult }); }
        catch { /* ignore */ }
      }
    ...
    console.log(`[Discovery] 完了: added=${added} updated=${updated} failed=${failed}`);
    return { ok: true, listingUrl, discovered: all.length, added, updated, skipped: failed, urls: results };
  }

  // ── 複数一覧ページの一括処理 ─────────────────────────────────
  static async discoverBatch(listingUrls, options = {}) {
    let totalAdded = 0, totalUpdated = 0;
    const results = [];

    for (let i = 0; i < listingUrls.length; i++) {
      console.log(`[Discovery] Batch ${i + 1}/${listingUrls.length}: ${listingUrls[i]}`);
      const r = await this.discoverTournamentUrls(listingUrls[i], options);
      results.push(r);
      if (r.ok) { totalAdded += r.added; totalUpdated += r.updated; }
      if (i < listingUrls.length - 1) await new Promise(r => setTimeout(r, 2000));
    }

    console.log(`[Discovery] Batch完了: totalAdded=${totalAdded} totalUpdated=${totalUpdated}`);
    return { ok: true, total: results.length, totalAdded, totalUpdated, results };
  }
}

window.UrlDiscoveryEngine = UrlDiscoveryEngine;

console.info(
  '%c[UrlDiscovery v1]%c URL Discovery Engine 初期化完了',
  'background:#2D7DD2;color:#fff;padding:2px 8px;border-radius:3px;font-weight:800;',
  'color:#1a5090;font-weight:600;'
);
